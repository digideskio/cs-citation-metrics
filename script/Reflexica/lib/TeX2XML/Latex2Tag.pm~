#  Author: Neyaz Ahmad/Sanjeev
package TeX2XML::handleCurlyDoller;
use Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(matchCurly matchEqualDoller rearrangeCurly rearrangeCurly deleteExtraCurly equationHandeling rearrangeBracket matchBracket);
#====================================================================================================================================================
sub matchCurly{
  my $TeXData=shift;
  ${$TeXData}=~s/\\{/<openCurly>/g;
  ${$TeXData}=~s/\\}/<closeCurly>/g;
  ${$TeXData}=~s/\{\s*\}/<dummyCurly>/g;
  ${$TeXData}=~s/([^\\])\\\[/$1<openBracket>/g;
  ${$TeXData}=~s/([^\\])\\\]/$1<closeBracket>/g;
...
sub matchBracket{
  ...
  my $counter = 1;
  while(${$TeXData} =~ m/\[([^\[\]]+)\]/g){
    ${$TeXData} =~ s/\[([^\[\]]+)\]/<bracket${counter}>$1<\/bracket${counter}>/g;
    $counter++;
  }
  return $TeXData;
}
#====================================================================================================================================================
sub rearrangeBracket{
  my $TeXData = shift;
  ${$TeXData} =~ s/<bracket[0-9]+>/\[/g;
  ${$TeXData} =~ s/<\/bracket[0-9]+>/\]/g;
  ${$TeXData} =~ s/<openBracket>/\\\[/g;
  ${$TeXData} =~ s/<closeBracket>/\\\]/g;
  return $TeXData;
