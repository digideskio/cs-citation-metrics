...
BEGIN{
    system ("clear");
    use Log::Log4perl;
    my $log_cfg = "/etc/EXT/Citation_Metrics/Config/log.cfg";
...
sub Get_Crossref_DOI{
    ...
    foreach my $bibID (@$bistructureIDRef){
        ...
        if(!exists $$bistructureRef{"$bibID"}{"DOI"}){ 
            ...
            #($bistructureRef, $DOI)= Get_DOI_from_DB($bistructureRef, $bibID);  #Check and run #update for statistics.
            
            if ($DOI){
                print "\t\tDOI found from Database\n" if(defined $cline);                
            }
