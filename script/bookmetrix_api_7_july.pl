...
#Author: Neyaz Ahmad
#Version 1.5
#perl /etc/EXT/Citation_Metrics/script/bookmetrix.pl -d "10.1007/978-0-387-77650-7_5" -l "/etc/EXT/Citation_Metrics/script/log" -r "STAT.txt" -c "cmd"

#perl /etc/EXT/Citation_Metrics/script/bookmetrix.pl -f "/etc/EXT/Citation_Metrics/Input/test/66_2014_712_test.xml" -l "/etc/EXT/Citation_Metrics/script/log" -r "/etc/EXT/Citation_Metrics/Statistics/STAT.txt" -c "cmd"
...
###### Parse fetched XML and get Structrured Hash Ref and ID "bistructureRef, bistructureIDRef"
    ...
    print "Get Input_Xml $file\n"  if(defined $cline);
    ($bistructureRef, $bistructureIDRef, $doi)=&Get_Input_Xml($file);

    my $xmlfilename="$file";
    $xmlfilename=~s/(^.*[\/\\]|^)(.*?)\.(xml)$/$2\.$3/is;
    $$bistructureRef{"FileName"}="$xmlfilename";
...
$$bistructureRef{"ValidDOI"}=0;

$$bistructureRef{"CrossRefAPICount"}=0;
...
######Insert DOI in Database 
insert_citation_data($bistructureRef, $bistructureIDRef);


use EnrichXML;
$bistructureRef=&Enrich_Springer_XML($file, $bistructureRef, $bistructureIDRef);
...
sub Get_Crossref_DOI{
    ...
    foreach my $bibID (@$bistructureIDRef){
        ...
        if(!exists $$bistructureRef{"$bibID"}{"DOI"}){ 
            ...
            ($bistructureRef, $DOI)= Get_DOI_From_CrossRef	($bistructureRef, $bibID);  #Check and run #update for statistics.
            
            if ($DOI){
                print "\t\tDOI found from CrossRef API\n" if(defined $cline);               
            }

            else{
               print "\n Serach DOI  from WebTools \n";
...
#                        insert_citation_data($bistructureRef, $DOI, $bibID);
                     }
                }else{
...
sub Get_DOI_From_CrossRef{
	...
	my $urlArgument  = "[\"$bistructureRef->{$bibID}{BibUnstructured}\"]";
	my $DOI          = "";

	#$urlArgument = "[\"Muthén B, Asparouhov T (2007) Wald test of mean equality for potential latent class predictors in mixture modeling. (Unpublished web note). http://www.statmodel.com \"]";
	...
	my $res     = $ua->request($req);
	my $webData = $res->decoded_content;
	...
	if ($res->{'_rc'} != 500) {
		...
		$webData    = $json->decode($webData);
		
		#print "\n webData decoded \n".Dumper $webData;

		#'query_ok' => bless( do{\(my $o = 1)}, 'JSON::PP::Boolean' ),
		#'query_ok' => $VAR1->{'results'}[0]{'match'}

		#print "\nmatch = $webData->{'results'}[0]{'match'} \n";

		if (defined $webData->{'results'}[0]{'match'} and $webData->{'results'}[0]{'match'}) {
		
			my $data = $webData->{"results"}[0]{"doi"};
			...
			$$bistructureRef{"$bibID"}{"DOISource"}="CrossRef-API";
			$$bistructureRef{"CrossRefAPICount"}=$$bistructureRef{"CrossRefAPICount"} + 1;
			print "\tDOI: $DOI\n" if(defined $cline);
			
#			if ($$bistructureRef{"$bibID"}{"Bibtype"} eq "BibUnstructured") {
#				$$bistructureRef{"DOIObtainedFromUnstructuredRef"}++;
#			}else{
#				$$bistructureRef{"DOIObtainedFromStructuredRef"}++;
#			}
...
sub Generate_Report_and_Statistics{
  ...
  ###Total DOI Found
  my $totalDOIFound= $$bistructureRef{SpringerLinkCount} + $$bistructureRef{CrossRefMetaCount} + $$bistructureRef{CrossRefWebCount} + $$bistructureRef{ResearchGateCount} + $$bistructureRef{SpringerLinkFuzzyCount} + $$bistructureRef{"CrossRefAPICount"};

  ###Total Obtain DOI (CrossRefMetaCount, CrossRefWebCount, ResearchGateCount, SpringerLinkFuzzyCount)
  my $totalObtainDOI=  $$bistructureRef{CrossRefMetaCount} + $$bistructureRef{CrossRefWebCount} + $$bistructureRef{ResearchGateCount} + $$bistructureRef{SpringerLinkFuzzyCount} + $$bistructureRef{"CrossRefAPICount"};
 ...
 $$bistructureRef{DOIObtainedDirectSources} = $$bistructureRef{CrossRefMetaCount} + $$bistructureRef{CrossRefWebCount} + $$bistructureRef{ResearchGateCount};
  ...
  #### Insert Citation statistics in database
  DBConnection::insert_CitationHtmlData($bistructureRef, $doi);
 ...
 #Over All Statistics based on CSV Formatter
  ...
  if(defined $webservice){
	...
	$JSON=~s/(\"\:)\{\"\$t\"\:\"(.*?)\"\}/$1\"$2\"/gs;
      ...
      $HtmlReport="${logFileLocation}\/${doi}\.html";  #check and delete
      $HtmlReport=~s/bbm\:/bbm\-/gs;                   #check and delete
      print "<html>$outFileString<\/html>";
    }
  }else{
      if(defined $file){
	  $xmlfilename="$file";
	  $xmlfilename=~s/(^.*[\/\\]|^)(.*?)\.xml$/$2\.html/is;
	  $HtmlReport="${logFileLocation}\/$xmlfilename";
      }else{
...
sub BibUnstructuredParse{
  ...
  my $runTagtoColorScrpit= "perl \"${SCRITPATH}\/Reflexica\/RefManager.pl\" -f \"$RefInputFile\" -o color -x A++ -a Bookmetrix -p -c springerbookmetrix";
...
sub Get_Input_Xml{
    ...
    my ($inputData, $boolValue) = &openFile($inputFile);
    my  $doi="";

    if($$inputData=~/<(ChapterDOI|ArticleDOI)>[\s]*([^<>]+?)[\s]*<\/\1>/){
	$doi=$2;
    }elsif($$inputData=~/<(BookDOI)>[\s]*([^<>]+?)[\s]*<\/\1>/){
	$doi=$2;
    }
    ...
    #Parse fetched XML Reference Data and get Structrured Hash Ref and Ref ID
    ($bistructureRef, $bistructureIDRef)=&Parser($$inputData, $bistructureRef, $bistructureIDRef, "nested");

#print "\n\nCopyright Year:", $$bistructureRef{"$bibID"}{"CopyrightYear"}, "\n\n";
#print "\n\nTYPE:", $$bistructureRef{"$bibID"}{"PaperType"}, "\n\n";
#exit;
	
    return ($bistructureRef, $bistructureIDRef, $doi);
}
